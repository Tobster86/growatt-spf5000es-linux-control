...
static void dump_hex_msg(const char* msg, uint8_t* pcData, uint16_t nLength)
{
    printf("%s", msg);
...
static void transmit_callback(struct sdfComms* psdcComms, uint8_t* pcData, uint16_t nLength)
{
...
static void objectReceived_callback(struct sdfComms* psdcComms, uint16_t nObjectID, uint16_t nLength, uint8_t* pcData)
{
    switch(psdcComms->lID)
    {
        case ID_BOB:
...
static void commandReceived_callback(struct sdfComms* psdcComms, uint16_t nCommandID)
{
...
static bool lengthCheck_callback(struct sdfComms* psdcComms, uint16_t nObjectID, uint16_t nLength)
{
    switch(nObjectID)
    {
...
static void error_callback(struct sdfComms* psdcComms, uint8_t cErrorCode)
{
    lErrorCallbacks++;
}

void test_comms_protocol()
{
    PRINT_DEBUG("---=== Comms protocol tests ===---\n");
